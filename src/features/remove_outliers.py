...
# as you can see from the plot above a lot of red dots as outliers if we discard we will be throwing a lot of data
# IQR method is distribution based outliers detector, we used all data in one pile without distinguish different exercises
# that's why we see a lot of red dots ( outliers)
...
# Insert LOF function
# LOF using unsupervised clustering (looking at neighbors, n=20 means looking at 20 neighboring data points)
# to detect outliers ( judging by the plots, not suitable for this case)
...
dataset, outliers, X_scores = mark_outliers_lof(df, outlier_cols, n=20)
for col in outlier_cols:
    plot_binary_outliers(
...
dataset, outliers, X_scores = mark_outliers_lof(
    df[df["label"] == label], outlier_cols, n=20
)
for col in outlier_cols:
    plot_binary_outliers(
        dataset=dataset, col=col, outlier_col="outlier_lof", reset_index=True
...
# --------------------------------------------------------------
# Choose method and deal with outliers (choose chauvenet as it's the best performer)
# --------------------------------------------------------------
